...
web/src/deepLink.js           readDeepLink() (?edit=<id>, ?edits), writeEditParam(id), clearEditParam()
...
### 4.5 Deep link
`?edit=<id>` (validated `^ve_[0-9a-z]{16}$`) → `{view:'aiEdit', id}`; `?edits` → list. `AiEditSession` writes/clears the param via
`history.replaceState` in effects (no setState). Auth: `onNeedAuth(retry)` only on a real 401.
...
// state initializers (the only in-place edits)
...
6. Analysis stages advance with `aria-current="step"`; discoveries fill live; closing and reopening via My edits or `?edit=<id>` resumes.
...
- `web/tests/smoke/ai-edit.smoke.mjs` (puppeteer-core from `server/node_modules`, Chrome from the puppeteer cache, Vite dev server,
  ...
  render progress → swap keeps `currentTime ±0.1 s`; export dialog with stale preview focuses "Update preview first"; `?edit=` refresh
