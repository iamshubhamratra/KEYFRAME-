  ...
  return (
    <div style={{ position: 'absolute', left: 0, right: 0, bottom: 60, height: 60, pointerEvents: 'none' }}>
      <div style={{ position: 'absolute', left: railL, bottom: 30, fontFamily: MONO, fontSize: 14, letterSpacing: '0.2em', color: rgba(theme.ink, 0.6), whiteSpace: 'nowrap' }}>
        SCN <span style={{ color: theme.accent }}>{String(index + 1).padStart(2, '0')}</span> / {String(count).padStart(2, '0')}
      </div>
      ...
      <div style={{ position: 'absolute', left: railL, right: railR, bottom: 16, height: 2, background: rgba(theme.ink, 0.16) }} />
      <div style={{ position: 'absolute', left: railL, bottom: 16, width: Math.max(0, headX - railL), height: 2, background: theme.accent }} />
      {new Array(count + 1).fill(0).map((_, i) => {
        const tx = railL + step * i;
        return <div key={i} style={{ position: 'absolute', left: tx - 1, bottom: 11, width: 2, height: tx <= headX ? 13 : 8, background: tx <= headX ? theme.accent : rgba(theme.ink, 0.28) }} />;
      })}
      <div style={{ position: 'absolute', left: headX - 7, bottom: 10, width: 14, height: 14, transform: 'rotate(45deg)', background: theme.accent, boxShadow: `0 0 16px ${rgba(theme.accent, 0.8)}` }} />
    </div>
  );
}
