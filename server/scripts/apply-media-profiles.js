...
for (const name of packs) {
  ...
  // THE FILMKIT FAMILY MEASURES ITS OWN CONTRACT. Every `film-*` pack's media block is
  // generated by scripts/gen-film-packs.js from the real box geometry in
  // services/film_beats.js (the hook's 936x460 device frame, the feature card's 910x562, the
  // montage tile's 455x292…), which is the same "measured, not estimated" rule the table below
  // follows — just measured at the composer that draws them rather than transcribed here. It is
  // already on disk, so this script must neither overwrite it nor call it missing: reporting it
  // as "falls back to a derived plan" would be plainly false.
  if (/^film-/.test(String(manifest.renderer || "")) && manifest.media && manifest.media.slotsByRole) {
    skipped++;
    continue;
  }
  ...
  // knows better, and that knowledge shows up as two things this script provably never writes:
  // a non-empty `note` (line ~607 emits `note: ""`) and a non-empty `placeholders` (line ~628
  // emits `[]`). Both are unreachable from the derivation, so their presence can only mean a
  // human measured this pack.
  //
  // Overwriting such a block does not "regenerate" it, it DELETES the measurement — silently,
  // because what is left still looks well-formed. Deep is the worked example: its
  // `deep-pocket-handset` placeholder cannot be expressed by the table at all (a role carries
  // ONE shape, and Deep's only portrait device shares its beat with landscape viewports), so
  // losing it means template_media emits no portrait bucket and crop_engine gets nothing to
  // analyse for that box. Its hook is likewise a measured 940x440, not the fraction's guess.
  //
  // If this script ever starts authoring notes, this detector stops discriminating — move to an
  // explicit marker then rather than letting the skip quietly become a no-op.
  const prior = manifest.media && typeof manifest.media === "object" ? manifest.media : {};
  const measured =
    (Array.isArray(prior.placeholders) && prior.placeholders.length > 0) ||
    Object.values(prior.slotsByRole || {}).some((s) => s && typeof s.note === "string" && s.note.trim());
  if (measured) { handMeasured.push(name); skipped++; continue; }

  const media = buildMedia(name, manifest);
  if (!media) { unknown.push(name); continue; }
  // THE AUTHORED STAGE SURVIVES REGENERATION. `media.stage` is not a quota this script derives
  // — it is the frame the composer's own slot pixels are measured on, and the only packs that
  // declare it are the ones that render at BOTH aspects, where `orientation` cannot imply it.
  // Rebuilding the block without it would silently return those packs to the resolution- and
  // aspect-dependent areaShare that template_media.js:456 exists to correct.
  if (prior.stage) media.stage = prior.stage;
...
// Name the packs this script deliberately did not touch. A silent skip is how a protection turns
// into a leak: nobody notices the guard stopped matching until the data is already gone.
if (handMeasured.length) {
  console.log(`[media-profiles] ${handMeasured.length} pack(s) carry a HAND-MEASURED media block and are left alone: ${handMeasured.join(", ")}`);
}

if (check) {
  console.log(`[media-profiles] check: ${skipped} up to date (${handMeasured.length} hand-measured), ${drift} drifted, ${unknown.length} unprofiled`);
  process.exit(drift ? 1 : 0);
}
console.log(`[media-profiles] wrote ${written}, unchanged ${skipped} (${handMeasured.length} hand-measured), unprofiled ${unknown.length} (of ${packs.length} packs)`);
