...
function deriveSkin(cfg, slug) {
  ...
  return {
    ...
    // `FM` is the article packs' mono slot (gutters, counters, the reading footer); the
    // library packs that predate them spell the same idea `codeFont`. Read both, or a pack
    // that declares a mono face silently renders its gutter in the body face.
    mono: cfg.FM ? parseStack(cfg.FM).family : (cfg.codeFont ? parseStack(cfg.codeFont).family : null),
    // `chrome:false` is a CLAIM ABOUT THE FRAME — no pack badge, no template name — and it
    // has to cross the derivation or the generated film wears our branding over the user's
    // content. Only ever false or absent; absent keeps the lockup, as every older pack expects.
    chrome: cfg.chrome === false ? false : undefined,
    // ARTICLE PACKS RUN LONG. A pack that declares the body/quote grammar is built to carry
    // an article, and 18 is the length that grammar was authored at; the library packs keep
    // the engine's 12. Emitted only when it differs, so no existing skin changes.
    maxScenes: (look.body || look.quote) ? 18 : undefined,
    em: emFor(FH.family),
...
const SKIN = {
  ...
  ${s.chrome === false ? "chrome: false," : ""}
  ${s.devanagari ? `devanagari: ${JSON.stringify(s.devanagari)},` : ""}
  ${s.maxScenes ? `maxScenes: ${s.maxScenes},` : ""}
  em: ${s.em},
...
function main() {
  ...
  // ---- SINGLE-CONFIG MODE ------------------------------------------------------------
  //   node scripts/gen-film-skins.js --config <file.pack-config.js> --slug <slug>
  //
  // The library drop is a BATCH: source/{mega,world}-pack-N.js holding many FilmKit.make()
  // calls, named by matching against the slug vocabulary in standalone/*.html. A single
  // pack shipped on its own satisfies neither half — its filename does not match the batch
  // pattern, and the drop it arrives in has no standalone/ to be named from (that directory
  // is the bundle's build output and is gitignored). Both are properties of the BATCH, not
  // of the derivation, so a one-file mode simply skips them: the config is named on the
  // command line, and deriveSkin/skinFile below are the same code the batch runs.
  const ci = argv.indexOf("--config");
  if (ci !== -1) {
    const cfgPath = path.resolve(argv[ci + 1] || "");
    const si = argv.indexOf("--slug");
    const slug = si !== -1 ? argv[si + 1] : path.basename(cfgPath).replace(/\.pack-config\.js$/, "").replace(/\.js$/, "");
    if (!fs.existsSync(cfgPath)) { console.error(`[film-skins] no such config: ${cfgPath}`); process.exit(1); }
    const captured = [];
    const R = (type, props, ...children) => ({ __el: true, type, props: props || {}, children });
    const sandbox = {
      console, Math, JSON, String, Number, Object, Array, Boolean, parseInt, parseFloat, isFinite, isNaN,
    ...
    sandbox.window.FilmKit = sandbox.FilmKit;
    vm.createContext(sandbox);
    vm.runInContext(fs.readFileSync(cfgPath, "utf8"), sandbox, { filename: path.basename(cfgPath), timeout: 20000 });
    if (!captured.length) { console.error(`[film-skins] ${path.basename(cfgPath)} made no FilmKit.make(cfg) call`); process.exit(1); }
    // SELECT BY GLOBAL, DO NOT SPLIT THE FILE. The article packs ship five templates per
    // config file, and cohorts 07-10 share a module-scope LOOK() helper that runs at
    // config-BUILD time. Splitting on the `global:` boundaries would leave that helper
    // behind and the split files would throw on load; evaluating the whole file and
    // picking the requested capture keeps every shared binding in scope for free.
    const gi = argv.indexOf("--global");
    const wantGlobal = gi !== -1 ? argv[gi + 1] : null;
    const cfg = wantGlobal ? captured.find((c) => c.global === wantGlobal) : captured[0];
    if (!cfg) {
      console.error(`[film-skins] ${path.basename(cfgPath)} has no FilmKit.make with global "${wantGlobal}" (found: ${captured.map((c) => c.global).join(", ")})`);
      process.exit(1);
    }
    cfg.__pack = path.basename(cfgPath);
    const derived = deriveSkin(cfg, slug);
    // The non-Latin fallback is declared per pack in manifests/<slug>.pack.json, not in the
    // config, so it has to be handed in. Without it on the skin, buildTheme cannot emit the
    // face or name it in the stacks and a Devanagari string renders as tofu.
    const di = argv.indexOf("--devanagari");
    if (di !== -1 && argv[di + 1]) derived.devanagari = argv[di + 1];
    const file = path.join(OUT_DIR, `${slug.replace(/-/g, "_")}.js`);
    fs.mkdirSync(OUT_DIR, { recursive: true });
    fs.writeFileSync(file, skinFile(cfg, slug, derived), "utf8");
    console.log(`[film-skins] wrote ${path.relative(process.cwd(), file)} from ${path.basename(cfgPath)} (global="${cfg.global}", ${(cfg.cams || []).length} cams)`);
    return;
  }
