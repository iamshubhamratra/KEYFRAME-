// FRAME PACK VIEW — how an installed frame pack (design system) is presented to
// the template gallery: a display label, a one-line vibe, its palette, its preview
// clip + poster, and which shelf it belongs on.
...
// display font + ground. Only the human display LABEL still comes from FRAME.md
// `name:` (the manifest holds the slug, not the title). Falls back to FRAME.md
// parsing when a pack ships no manifest.
...
function presentPack(name, defaultPack) {
  return {
    name,
    default: name === defaultPack,
    ...packMeta(name),
    ...mediaUrls(name),
