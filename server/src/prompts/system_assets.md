...
- **A concept you can't photograph → shoot the PEOPLE using it, not the concept.** For a software / AI / digital / abstract subject (e.g. "AI note-taking", "data sync", "automation"), a concept query returns junk — matrix code, circuit boards, random dashboards. Query the real humans + real workspace + a real action instead ("focused woman laptop notes desk", "diverse team meeting laptops office", "hands writing in a notebook closeup"). **Banned words** (they pull code/matrix/circuit stock every time): "digital screen", "AI interface", "data", "software", "technology background", "cyber", "futuristic" — name a real object, person, or place instead.
