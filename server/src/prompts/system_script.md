...
7b. **User-uploaded assets:** when the inputs include a USER ASSET INVENTORY, those are the user's own materials and the pipeline supplies them automatically — the film should be built AROUND them. Plan at least one showcase scene per 2 uploaded screenshots/UI images: a dashboard or app screenshot earns a feature/how scene whose `visualDirection` showcases "the user's real product UI in a device frame"; product/team/marketing photos earn context/proof scenes that feature them. When a brand logo is provided, the opening and CTA scenes should leave visual room for a small brand mark (do not write the logo into `onScreenText`). Do NOT add `assetNeeds` duplicating uploaded material — uploads arrive on their own; only request stock for gaps the inventory leaves.
