  ...
  const worker = async () => {
    for (;;) {
      ...
      try {
        ...
        // DIMENSIONS ARE PART OF THE ANSWER, NOT JUST AN INPUT. An asset that arrived
        // with no width/height is probed here anyway to pick the crop window — and every
        // consumer downstream (asset_fit's shape scoring, template_media's slot matching,
        // the preflight resolution gate) needs the same two numbers. Leaving them on the
        // floor is what made an unmeasured asset invisible to shape-aware selection.
        const dims = (a.width && a.height) ? { width: a.width, height: a.height } : await probeDims(abs);
        if (dims && dims.width > 0 && dims.height > 0 && !(a.width && a.height)) {
          a.width = dims.width; a.height = dims.height;
          if (!a.ratio) a.ratio = dims.width / dims.height;
        }
        const r = await analyzeImage(abs, { aspects, kind, dims });
        if (r.focus) {
          a.cropFocus = r.focus;
...
async function probeDims(absPath) {
  try {
    const { ffprobeImage } = require("./asset_sources/util"); // repointed: this repo exports it from util, not the index barrel
