...
// Stamp `role` onto every scene of a script/storyboard, in place. Idempotent, and it
// never overwrites a role that is already canonical.
function stampRoles(scenes) {
  const list = Array.isArray(scenes) ? scenes : [];
  list.forEach((sc, i) => {
    if (!sc || (typeof sc.role === "string" && ROLE_SET.has(sc.role))) return;
    sc.role = roleOf(sc, { index: i, total: list.length });
  });
  return list;
}
...
module.exports = { ROLES, coerceRole, roleOf, stampRoles, isShowcase, showcaseTargets, SHOWCASE_ROLES };
