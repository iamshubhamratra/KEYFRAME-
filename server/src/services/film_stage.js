...
const { tempoOf, VERTICAL_FILM_SCENE_CAP } = require("./pacing");
...
// 6 = PORTRAIT COMPOSITION PASS: the 9:16 beats stop being a copy block at one authored `top`
//     plus a picture pinned at a second. film_portrait.js derives the geometry (the plate takes
//     the ASSET'S aspect; its height comes from the room the copy leaves inside a measured safe
//     band), seven compositions share the media beats so a two-minute film never draws one beat
//     the same way twice, every beat gains a rail built from copy the scene already carries, and
//     builders may emit their own timeline tweens (`built.tweens`) for the ken-burns and light
//     sweeps a static 9:16 card never had. LANDSCAPE IS BYTE-IDENTICAL — proved by
//     scripts/golden-composers.js, which this pass also finally writes.
const ENGINE_REV = 6;
...
// Attribute-safe font stack: see the note in buildTheme. Converts any double quote in a
// family name to a single quote so the stack can be interpolated into style="..." unharmed.
const q = (stack) => String(stack).replace(/"/g, "'");
// Splice the pack's declared Devanagari face in before the generic tail, so a Devanagari
// codepoint resolves to a real face instead of the browser's last-resort box.
function dev(stack, skin) {
  if (!skin.devanagari || !isBundled(skin.devanagari)) return stack;
  const parts = String(stack).split(",");
  return [parts[0], ` '${skin.devanagari}'`, ...parts.slice(1)].join(",");
}

function buildTheme(skin, brandSkin) {
  // NON-LATIN FALLBACK IS PART OF THE STACK, NOT A SPARE TYRE.
  // Every article pack declares a Devanagari face in its manifest, and the handoff is explicit
  // that without it a Hindi string renders as tofu. Bundling the face is not enough: it has to
  // be EMITTED as an @font-face and NAMED in the stacks, or nothing can reach it. It goes last,
  // so Latin text is unaffected and only codepoints the Latin face lacks fall through to it.
  const faces = [skin.display, skin.body, skin.mono, skin.devanagari].filter(Boolean).filter(isBundled);
  ...
  const theme = {
    ...
    // SINGLE QUOTES ONLY, ALWAYS. Every one of these stacks is interpolated into an HTML
    // `style="..."` attribute, so ONE double quote anywhere in a family name closes the
    // attribute early and the browser silently discards every declaration after the
    // font-family — font-size, colour, the lot. It cost the article pack's CTA its type size:
    // a `"Helvetica Neue"` fallback truncated the style attribute and the 112px headline
    // rendered at the 16px default. None of the library skins carried a quoted fallback, so
    // nothing caught it for 105 packs. Quoting style is semantically irrelevant in CSS, so
    // normalising is free — and it belongs HERE, at the one place the stacks are built,
    // rather than at each of the ~40 sites that emit them.
    displayStack: q(dev(`'${skin.display}', ${skin.displayFallback || "Georgia, serif"}`, skin)),
    bodyStack: q(dev(`'${skin.body}', ${skin.bodyFallback || "system-ui, sans-serif"}`, skin)),
    monoStack: q(dev(skin.mono ? `'${skin.mono}', ui-monospace, monospace` : "ui-monospace, SFMono-Regular, Menlo, monospace", skin)),
...
// THE FIELD THIS USED TO READ IS NOT ON THE OBJECT IT IS GIVEN.
//
// `scene.onScreenText` is a SCRIPT field. services/storyboard.js alignToScript
// builds each storyboard scene as `{...donor, id, start, duration, headline,
// subtext, bullets, voiceover}` — the script's display lines are folded INTO
// headline/subtext/bullets and `onScreenText` is never copied across. So on the
// graph and project paths this array was empty on every scene, and every
// FilmKit feature row in the studio fell through to splitting the VOICEOVER.
//
// Which is the whole defect the pacing work exists to fix, in miniature: the
// picture's list copy was a function of the narration, so choosing a quicker
// pace — which cuts narration by design — cut the feature rows with it. The
// storyboard's own authored labels, and everything services/content_density.js
// mines out of the brief, sat in `scene.bullets` where nothing on this path
// looked.
//
// Order is deliberate: the script's lines if they somehow survived, then the
// authored/derived labels, and only then the narration as the last resort it
// always should have been.
function featureLines(scene, n) {
  let src = Array.isArray(scene.onScreenText) ? scene.onScreenText.filter(Boolean).map(String) : [];
  if (Array.isArray(scene.bullets)) src = src.concat(scene.bullets.filter(Boolean).map(String));
...
function plate(theme, { asset, scrollId, natH, tint }) {
  if (asset && asset.path) {
    ...
    return scrollId && natH
      ? `<div id="${scrollId}" style="position:absolute;left:0;right:0;top:0;height:${X(natH)};"><img src="${esc(asset.path)}" alt="${esc(asset.alt || "")}" style="width:100%;height:100%;${AF.fitCss(asset)}display:block;"></div>`
...
function backingPlate(asset, top, height) {
  return `<div style="position:absolute;left:0;right:0;top:${top};height:${height};overflow:hidden;-webkit-mask-image:${PLATE_MASK};mask-image:${PLATE_MASK};">
      <img src="${esc(asset.path)}" alt="${esc(asset.alt || "")}" style="width:100%;height:100%;${AF.fitCss(asset)}opacity:.22;filter:blur(${X(9)});transform:scale(1.06);display:block;">
    </div>`;
}
...
function archetypeFor(scene, i, total, look) {
  ...
  // ARTICLE BEATS — routed ONLY when the pack actually declares the look. The 105 skins
  // that shipped before these builders existed have no `look.body` / `look.quote`, and a
  // quote scene has always drawn as a statement for them; widening the route unconditionally
  // would silently restyle every one of those films.
  if (look && look.quote && (k === "quote" || /quote|testimonial/.test(p))) return "quote";
  if (look && look.body && (k === "body" || /body|paragraph|prose|explain|argument/.test(p + k))) return "body";
...
// `body` and `quote` are deliberately ABSENT: both are pure-type beats in the kit, and a
// shot landed on one would cover the paragraph it exists to make readable.
...
// A labelsOnly MONTAGE RENDERS NO PICTURES, SO IT MUST NOT BE SEATED ANY.
//
// `look.montage.labelsOnly` makes the wall a type-only comparison — 50 of the 155 skins declare
// it, the whole article cohort — and the builder ignores `sceneAssets` entirely on that path.
// The seating loop did not know: montage is first in CAN_SHOW and the leftovers loop's FIRST
// preference is an empty montage, so on every one of those packs the two highest-scoring
// leftover shots were seated on beats that discard them. Measured on a 16-scene film: two
// pictures collected, scored, cropped and assigned, then drawn nowhere. Consulting the skin's
// own declaration hands them to a beat that will actually show them.
const showsMedia = (skin, arch) => !(arch === "montage" && skin && skin.look && skin.look.montage && skin.look.montage.labelsOnly)
  && !(skin && skin.media === false);
const capacityOf = (arch, skin) => (showsMedia(skin, arch) ? (SHOT_CAPACITY[arch] || 0) : 0);
...
function build(skin, { storyboard, dims, framePack, captionCues, assets, brandSkin = null, localized = null, seedKey = null } = {}) {
  ...
  // HOW MANY BEATS THIS SKIN DRAWS. 12 is the library default — the length the original
  // seventy templates were authored at — not a property of the engine. The article packs run
  // 18, and a hard 12 here silently dropped their last six beats: the pipeline had already
  // cut and mixed narration for scenes this cap then refused to draw, so those words landed
  // over a neighbour's frame. A skin that knows its own length says so.
  // A VERTICAL FILM DRAWS MORE OF THEM. The deck's authored length is what held a 180s portrait film
  // at 10s a slide, so pipeline.js sceneCapFor plans vertical films against pacing's
  // VERTICAL_FILM_SCENE_CAP instead. This slice is the backstop for that decision and must not
  // disagree with it: the scenes past the cap have already been narrated and mixed by the time they
  // reach here, so slicing them away drops their words onto a neighbour's frame — the exact defect
  // recorded above. Landscape keeps the skin's own length.
  const beatCap = Math.max(1, H > W
    ? Math.max(Number(skin.maxScenes) || 12, VERTICAL_FILM_SCENE_CAP)
    : (Number(skin.maxScenes) || 12));
  const scenes = Array.isArray(sb.scenes) && sb.scenes.length
    ? sb.scenes.slice(0, beatCap)
  ...
  // `energy` feeds exactly two things — the camera entrance-push magnitude in
  // camVarsFor, and the ambient sine-drift amplitude — and BOTH are spatial
  // camera/ambient quantities. So it takes the tempo's CAMERA dial.
  //
  // It used to divide by `tempo.motion`, which is the ENTRANCE dial. That was
  // harmless while nothing ever wrote `storyboard.pacing` and tempoOf always
  // returned neutral, but once the pacing engine feeds it the wiring inverts the
  // design: Relaxed declares `camera: 1.0` and `ambient: 1.0` yet, through
  // motion, cut this skin's camera travel from 1080px to 864px and shrank the
  // ambient drift with it — the two axes services/pacing.js most explicitly
  // says a mode must not touch, moved by a mode that said it would not.
  //
  // Multiplying by camera keeps every intent intact: neutral tempo leaves
  // `skin.energy` exactly as it was, a faster mode pushes a little further
  // (matching camPush, which is a DISTANCE and scales up), and the no-voiceover
  // tilt still lifts it — that tilt's own comment says "further travel in less
  // time reads as energy", i.e. it was always a camera idea.
  const energy = (skin.energy || 1) * (tempo.camera || 1);
  ...
  const baseArch = scenes.map((sc, i) => archetypeFor(sc, i, scenes.length, look));
  ...
  const displayIdx = scenes.map((_, i) => i).filter((i) => CAN_SHOW.has(baseArch[i]) && capacityOf(baseArch[i], skin) > 0);
  ...
  if (displayIdx.length && shots.length) {
    ...
    const room = (i) => sceneShots[i].length < capacityOf(baseArch[i], skin);
    ...
    for (const a of leftovers) {
      ...
      if (best == null) best = displayIdx.find((i) => capacityOf(baseArch[i], skin) >= 4 && !holds(i, a));
  ...
  // THE PORTRAIT PATTERN ROTATION. film_beats picks one of several 9:16 compositions per
  // beat so a two-minute film that draws `statement` five times draws five different
  // frames. Offsetting the rotation per job — from the same seeded PRNG the camera order
  // uses, so a re-render is byte-identical — means two films on one pack also differ.
  const patternSeed = Math.floor(rnd() * 97);
  ...
  scenes.forEach((scene, i) => {
    ...
    if (!m && CAN_SHOW.has(arch) && sceneAssets.length) {
      ...
      // A FEATURE HOLDING SEVERAL PICTURES BECOMES A WALL — BUT ONLY WHERE A WALL SHOWS THEM.
      //
      // This promotion is how surplus imagery reaches the screen, and on the 50 skins whose
      // montage is `labelsOnly` it did the opposite: the wall those skins draw is a type-only
      // comparison that never reads `sceneAssets`, so promoting a feature that had earned two
      // pictures deleted both. (Seating no longer routes shots INTO a labelsOnly montage; this
      // is the same defect one step downstream, and it is the one that actually shipped —
      // measured on bound_volume, two feature beats became empty label walls.)
      //
      // Where the wall cannot show them the beat stays a feature and keeps TWO, which the
      // portrait `duo` composition draws as an offset pair. Landscape still takes one, exactly
      // as before, because bFeature's wide branch reads sceneAssets[0] alone.
      else if (sceneAssets.length >= 2 && arch !== "hook") {
        if (showsMedia(skin, "montage")) { arch = "montage"; sceneAssets = sceneAssets.slice(0, 4); }
        else sceneAssets = sceneAssets.slice(0, isWide() ? 1 : 2);
      }
    ...
    const ctx = {
      id: `s${i + 1}`, T, L, i, index: i, patternSeed, isLast: i === scenes.length - 1, track: 10 + i, beat: key,
      // The beat COUNT, not just this beat's index: the article `body` beat draws a
      // "nn / NN" reading footer, and a builder that only knows its own index cannot
      // say how far through the film the reader is.
      total: scenes.length,
      // THE SCENE ITSELF. The ten interaction builders reach their layout through `shell()`,
      // which was handed a title string and nothing else — so those beats could never draw the
      // sub line or the display list the scene actually carries, whatever the frame had room
      // for. Every builder already receives `scene` as its first argument; putting it on the
      // context is what lets the SHARED chrome read it too.
      scene,
    ...
    // A BUILDER MAY EMIT ITS OWN TWEENS.
    //
    // `scroll` above was the only channel a beat had into the timeline, so the one motion a
    // portrait plate could carry was a vertical pan — every other move (a slow push on a still
    // capture, a light sweep across a screen) had nowhere to be expressed and the 9:16 beats
    // read as static cards under a moving camera. A builder now returns `tweens: [string]` and
    // they are appended verbatim, under the same two rules the rest of this timeline obeys:
    // the element's RESTING state is the finished frame, and nothing may be time-dependent
    // outside GSAP, so any seek lands on the right pixels.
    for (const tw of (Array.isArray(built.tweens) ? built.tweens : [])) {
      if (typeof tw === "string" && tw.trim()) s.push(tw);
    }
    if (!ctx.isLast) {
  ...
  const iconHtml = typeof skin.icon === "function"
    ...
    : "";
  // CHROME OFF. `chrome:false` means the film carries the USER'S content and nothing else —
  // no pack badge, no template name. Every article pack sets it, because a viewer who sees
  // "Margin Notes" in the corner of a film about someone else's product is looking at our
  // branding on their video. Emitting an empty string (rather than a hidden clip) keeps the
  // element out of the document entirely, so nothing can un-hide it downstream.
  const chrome = skin.chrome === false ? "" : `<div id="fk-chrome" class="clip" data-start="0" data-duration="${D}" data-track-index="92" data-layout-allow-occlusion style="pointer-events:none;">
...
// ALIAS FOR A NAME A WORLD USES BUT DOES NOT DECLARE.
// A World is emitted verbatim and must close over only R and rgba. One authored pack
// (bound-volume) also references onAccent, which is declared inside a DIFFERENT World in
// the same source file - a copy-paste omission. FKdraw wraps the call in a try/catch, so the
// ReferenceError was swallowed and that pack rendered a completely empty backdrop while
// passing lint, the runtime smoke and every frame gate. Declaring the alias here fixes it
// without touching the authored function, which is the rule for this library. theme is
// always THEME inside this scope, so this is the same value the sibling World computes.
var onAccent = THEME.currentBg === THEME.accent;
...
module.exports = {
  ...
  // Exported for film_beats.bCta: an empty logo slot draws the PACK'S OWN MARK in a filled
  // badge rather than nothing, so the closing frame is never a bare headline.
  renderIcon,
