...
const { GSAP_CDN, r, esc, hexToRgb, longestWord, bullets, logoAssetOf, grainUri } = require("./composer_kit");
...
function bCta(scene, ctx, logo) {
  ...
  const html = `${open(id, ctx)}<div class="du-safe">
    ...
    <div class="du-wordmark" style="font-size:${r(Math.min(11, fitCap(word)))}cqw;margin-top:3cqw;">${chars}</div>
...
function styleBlock(theme, portrait) {
  ...
  * { box-sizing:border-box; margin:0; padding:0; }
  ...
  .du-wordmark { display:flex; flex-wrap:wrap; justify-content:center; text-align:center; gap:0 0.26em; max-width:92%; font-family:${theme.displayStack}; font-weight:800; font-size:11cqw; letter-spacing:-0.02em; color:${theme.text}; }
