...
// `scene.bullets` is read BEFORE falling back to chopping up the subtext, for
// the same reason featureLines (services/film_stage.js) now reads it: on the
// graph and project paths `onScreenText` is a SCRIPT field that storyboard
// alignToScript never copies onto the scene, so this list was reached only by
// splitting a sentence — and a sentence chopped on its semicolons makes
// fragments, not labels. The storyboard's own authored labels live in `bullets`.
function bullets(scene, n) {
  let list = Array.isArray(scene.onScreenText) ? scene.onScreenText.filter(Boolean).map(String) : [];
  if (!list.length && Array.isArray(scene.bullets)) list = scene.bullets.filter(Boolean).map(String);
