...
function plateOk(a) {
  ...
  return a.source === "website" || a.source === "blog" || src.startsWith("library")
...
function bpTitle(scene, ctx) {
  ...
  <div class="safe" style="align-items:flex-start;text-align:left;padding-left:${land ? 12 : 9}%;">
    ...
    <div style="position:relative;max-width:${colW}cqw;">
    ...
    <div class="body" id="${id}-sub" style="opacity:0;margin-top:1.6cqw;max-width:${colW}cqw;">${sub}</div>
  </div></div>`;
  const s = [
    ...
    `tl.fromTo("#${id}-pencil",{x:0},{x:"${land ? 43.5 : colW - 3}cqw",duration:0.6,ease:"power2.out"},${r(T + 2.5)});`,
...
function bpFigure(scene, ctx) {
  ...
  <div class="safe" style="flex-direction:${land ? "row" : "column"};gap:${land ? 6 : 3.5}cqw;text-align:left;">
    ...
    <div style="max-width:${land ? 40 : 82}cqw;${land ? "" : "text-align:center;"}">
      <div class="fig" id="${id}-figl" style="opacity:0;">${esc(fig)}</div>
      <h2 class="display" id="${id}-head" style="font-size:${land ? 3.6 : 5.6}cqw;margin-top:1cqw;opacity:0;">${hl(scene, theme.amber)}</h2>
...
function bpFlowchart(scene, ctx) {
  ...
  // Portrait stacks the steps vertically — the horizontal 3-box row overflowed a
  // 9:16 sheet (the third box rendered off-canvas). Arrows point down instead.
  const arrow = land
    ? () => `<svg class="${id}-arrow" width="90" height="40" viewBox="0 0 90 40" style="overflow:visible;flex:0 0 auto;">
    ...
    : () => `<svg class="${id}-arrow" width="40" height="64" viewBox="0 0 40 64" style="overflow:visible;flex:0 0 auto;">
      <line class="draw" pathLength="100" x1="20" y1="4" x2="20" y2="42" stroke="${theme.cyan}" stroke-width="3"/>
      <path class="${id}-ah" d="M8 38 L20 60 L32 38 Z" fill="${theme.cyan}" opacity="0"/></svg>`;
...
function bpRevisions(scene, ctx) {
  ...
  const rowHtml = lines.map((t, i) => `<div style="position:relative;margin-top:${i ? "0.9cqw" : "1.8cqw"};">
      <div class="display ${id}-line" style="font-size:${land ? 3.6 : 5.4}cqw;opacity:0;color:${theme.faint};">${esc(String(t).toUpperCase())}</div>
  ...
  <div class="safe" style="align-items:flex-start;text-align:left;padding-left:14%;">
    ...
    <div class="mono" id="${id}-note" style="opacity:0;margin-top:1.3cqw;font-size:${land ? 1.05 : 1.8}cqw;letter-spacing:0.2em;color:#7E9CC2;">FILED PER SPEC 001 · NO FURTHER REVISIONS EXPECTED</div>
  </div></div>`;
...
function bpCta(scene, ctx) {
  ...
  <div class="safe">
    <div style="position:relative;">
      ...
      <div class="display" id="${id}-mark" style="font-size:${land ? 6.4 : 9.4}cqw;opacity:0;">${esc(mark)}</div>
    </div>
    <div style="position:relative;">
      <div class="body" id="${id}-tag" style="opacity:0;margin-top:0.8cqw;">${tag}</div>
      <svg width="480" height="14" viewBox="0 0 480 14" style="position:absolute;left:50%;margin-left:-${land ? 12.5 : 19}cqw;bottom:-0.7cqw;width:${land ? 25 : 38}cqw;overflow:visible;">
...
function bpPlate(scene, ctx, asset) {
  ...
  <div class="safe" style="flex-direction:${land ? "row" : "column"};gap:${land ? 6 : 3}cqw;text-align:left;">
    ...
    <div style="max-width:${land ? 36 : 82}cqw;${land ? "" : "text-align:center;"}">
      <div class="fig" id="${id}-figl" style="opacity:0;">${esc(fig)}</div>
      <h2 class="display" id="${id}-head" style="font-size:${land ? 3.4 : 5}cqw;margin-top:1cqw;opacity:0;">${hl(scene, theme.amber)}</h2>
...
function chromeHtml(theme, W, H, title, author) {
  return `
...
function styleBlock(theme, land = true) {
  // PORTRAIT OVERRIDES: everything is sized in cqw (container WIDTH), so a
  // 1080x1920 canvas renders the landscape sizes 44% smaller relative to the
  // frame and reads like a shrunken landscape sheet. The block appended at the
  // end re-scales type/furniture for 9:16 (same specificity, later wins).
  const portrait = land ? "" : `
  .safe { padding:10% 7% 14%; }
  .display { font-size:8.4cqw; }
  .label, .fig { font-size:1.9cqw; }
  .body { font-size:2.6cqw; }
  .caret { width:1.1cqw; height:3.3cqw; vertical-align:-0.45cqw; }
  .stamp { font-size:4.8cqw; border-width:0.6cqw; padding:1.3cqw 3.6cqw; }
  .dim-label { font-size:1.7cqw; }
  .pencil { width:3.6cqw; }
  .fbox { width:64cqw; }
  .fbox .fb-t { font-size:3.2cqw; }
  .fbox .fb-s { font-size:2cqw; min-height:0; }
  .fbox .pbar { height:0.7cqw; }
  .fcheck { width:3.4cqw; height:3.4cqw; right:1.6cqw; top:1.6cqw; }
  .btick { width:1.8cqw; height:1.8cqw; }
  .cta { font-size:3.6cqw; padding:2cqw 4.6cqw; gap:1.6cqw; }
  .bp-plate-head { font-size:1.8cqw; }
  #ch-xy { font-size:1.4cqw; }
...
function buildComposition({ storyboard, dims, framePack, captionCues, assets } = {}) {
  ...
  // REAL website screenshots first (the user's own product beats any stock/vector),
  // then best-scored; each is placed on a `figure` scene (the flexible slot), so
  ...
  // Title-block DRAWN BY carries the film's actual brand (the site the
  // screenshots came from) instead of studio-lore filler like "12 AGENTS".
  ...
  const indexHtml = [
    ...
    `<style>`, styleBlock(theme, W >= H), `</style>`,
    `</head>`, `<body>`,
