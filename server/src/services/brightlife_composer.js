...
function brightTheme(framePack, sb) {
  ...
  // Identity system: when another LIGHT pack reaches this composer via the
  // render3d auto-dispatch, its own accents lead the color system (white ground +
  // bright grammar stay, but the film reads as THAT pack). Every light pack used
  // to collapse to one fixed indigo/violet world here. The brightlife pack itself
  // (and any pack without usable accents) keeps the authored house palette.
  const own = framePack && framePack !== "brightlife"
    ? (base.accents || []).filter((h) => /^#[0-9a-fA-F]{6}$/.test(String(h)) && lum(h) < 200)
    : [];
  const HOUSE = ["#6366F1", "#8B5CF6", "#06B6D4", "#EC4899"];
  const accents = own.length >= 2 ? [...own, ...HOUSE.filter((h) => !own.includes(h))].slice(0, 4) : HOUSE;
  const PALETTE = own.length >= 2
    ? [...accents, ...["#3B82F6", "#10B981", "#F59E0B", "#A855F7", "#F43F5E", "#22C55E"].filter((h) => !accents.includes(h))].slice(0, 10)
    : ["#6366F1", "#8B5CF6", "#3B82F6", "#06B6D4", "#EC4899", "#F43F5E", "#10B981", "#22C55E", "#F59E0B", "#A855F7"];
  ...
  return {
    ...
    // Emphasis-word gradient: the pack's own lead accents when it brought a
    // usable palette; the indigo→violet→purple house ramp otherwise.
    emphA: accents[0], emphB: accents[1] || accents[0], emphC: accents[2] || accents[1] || accents[0],
    displayStack,
...
function chipsFor(scene, treatment) {
  ...
  // A period splits only when it ends a clause — not inside a number, so
  // "99.999% Uptime" stays ONE chip instead of "99" + "999% Uptime".
  const words = String(scene.subtext || scene.headline || "").split(/[,;]|\.(?!\d)/).map((s) => s.trim()).filter((s) => s && s.length <= 22);
  words.forEach(push);
  const defaults = {
...
function sceneOverlay(scene, i, total, ctx) {
  ...
  const baseBig = (isHero ? (land ? 148 : 94) : treatment === "cta" ? (land ? 118 : 78) : (land ? 90 : 58)) * sc;
  // Never let the widest word overflow its line box: .khead carries
  // overflow-wrap:anywhere as a last resort, which BREAKS MID-WORD when the
  // font outgrows the canvas ("US$1.9 / tn" on 9:16). Cap the size so the
  // longest word always fits the padded width (display glyphs avg ≈0.62em).
  const availW = dims.width * (centered ? 0.82 : 0.86);
  const capPx = Math.floor(availW / (Math.max(4, longestWord) * 0.62));
  const big = Math.min(Math.round(baseBig * measure), capPx);
  ...
  const pad = centered ? "0 9%" : land ? "0 7% 9%" : "0 7% 13%";
...
const cam=new THREE.PerspectiveCamera(42,W/H,0.1,140);
...
for(const sc of SCENES){
  ...
  mine.forEach((spec,idx)=>{
    ...
    if(spec.role==="hero"){pl.position.set(land()?2.1:0, land()?1.35:1.5, 0); pl.rotation.y=land()?-0.14:0;}
    else if(spec.role==="feature"){const n=mine.length;const spread=n>1?(idx-(n-1)/2):0;
      if(land()){pl.position.set(1.9+spread*3.0, 1.5+(idx%2?-0.34:0.34), -idx*1.5);pl.rotation.y=-0.15-spread*0.16;}
      else{
        // Portrait frustum is too narrow for ANY 3-across spread (a w=2.6 card
        // fills the whole visible width) — stack a slightly-smaller COLUMN down
        // the tall axis with a small alternating x jitter for life.
        pl.scale.setScalar(0.78);
        pl.position.set((idx%2?0.35:-0.35), 3.0-idx*2.05, -idx*0.6);
        pl.rotation.y=(idx%2?-0.08:0.08);
      }}
    else if(spec.role==="side"){pl.position.set(land()?3.1:0, land()?1.3:1.6, -0.3);pl.rotation.y=-0.22;}
...
function cameraFor(type,lt,dur){
  ...
  // Portrait viewport is ~half as wide: full-width camera pans sweep the
  // plates off-frame, so horizontal travel is damped on 9:16.
  const xk=land()?1:0.45;
  if(type==="hook")      return {px:lerp(-0.6,0.7,e)*xk+hx, py:0.3+hy, pz:lerp(12.5,8.2,e), tx:0, ty:0, tz:0, roll:lerp(0.01,-0.008,e)};
  if(type==="problem")   return {px:lerp(-2.2,2.2,e)*xk+hx, py:hy*1.3, pz:9.6, tx:lerp(0.7,-0.7,e)*xk, ty:0, tz:-1, roll:lerp(-0.012,0.012,e)};
  if(type==="solution")  return {px:lerp(1.1,-0.3,e)*xk+hx, py:0.6+hy, pz:lerp(10.4,7.0,e), tx:lerp(0.6,-0.3,e)*xk, ty:0.5, tz:0, roll:0};
  if(type==="features")  return {px:lerp(-3.0,3.0,e)*xk+hx, py:0.8+hy, pz:8.6, tx:lerp(-1.5,1.5,e)*xk, ty:0.5, tz:-0.6, roll:lerp(0.016,-0.016,e)};
  if(type==="benefits")  return {px:lerp(1.9,-0.3,e)*xk+hx, py:0.55+hy, pz:lerp(8.8,6.8,e), tx:lerp(0.8,-0.2,e)*xk, ty:0.5, tz:0, roll:0};
...
function assignPlates(scenes, sceneWindows, assets) {
  ...
  const used = new Set();
  // WHICH PICTURE BELONGS ON *THIS* CARD. `rank` above is entirely film-global —
  // screenshot-ness and the vision verdict, nothing about what the beat is
  // SAYING — so every card simply popped the next item off that one list, and on
  ...
  // on the 7-beat fixture: 2 of 10 cards showed something the narration
  // mentioned, and the footer capture the vision pass had REJECTED landed on a
  // feature beat. pickForScene reads the same fields the pipeline already
  // produced (`sees`, sectionType, the beat's purpose) and returns null when
  // nothing is even loosely about this beat, so a thin pool still fills every
  ...
  sceneWindows.forEach((w) => {
    ...
    if (type === "solution") {
      ...
      for (let k = 0; k < 3; k++) { const a = take(sc); plates.push({ scene: w.i, role: "feature", tex: a ? a.path : null, ui: UIKINDS[k % 3], aspect: a ? ratio(a) : 1.6, w: 2.6, ci: k }); }
      ...
      if (avail() > 0) { const a = take(sc); plates.push({ scene: w.i, role: "side", tex: a.path, aspect: ratio(a), w: 2.4, ci: 1 }); }
    }
...
function buildComposition({ storyboard, dims, framePack, captionCues, assets } = {}) {
  ...
  const catalogue = (sb.title || "KEYFRAME").toUpperCase().slice(0, W >= H ? 34 : 18);
  bodyHtml.push([
  ...
  const indexHtml = [
    ...
    `.khead { margin:0; font-family:${theme.displayStack}; font-weight:700; line-height:0.98; letter-spacing:-0.032em; color:${theme.ink}; overflow-wrap:anywhere; text-wrap:balance; }`,
    ...
    `.kcat { position:absolute; left:${W >= H ? 58 : 40}px; right:${(W >= H ? 58 : 40) + px(120)}px; bottom:42px; font-size:${px(12)}px; letter-spacing:.2em; text-transform:uppercase; color:${theme.faint}; white-space:nowrap; overflow:hidden; text-overflow:ellipsis; }`,
