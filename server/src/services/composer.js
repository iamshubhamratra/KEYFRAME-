...
function quickCheck(indexHtml, metaJsonStr, { width, height, duration, assets, enforceVectors = true, sceneCount: sceneCountOpt } = {}) {
  ...
  if (enforceVectors) {
    ...
    // Require at least min(4, provided) DISTINCT provided images to actually
    ...
    if (imageAssets.length) {
      ...
      const neededCount = Math.min(4, imageAssets.length); // "as many assets as fit" — 2 was passable with 9 fetched
