...
function blTitle(scene, ctx) {
  ...
  const html = `${open(id, ctx)}<div class="safe">
    ...
    <svg id="${id}-fl" width="420" height="30" viewBox="0 0 420 30" style="margin-top:1.6cqw;overflow:visible;max-width:24cqw;">
...
function blCards(scene, ctx) {
  ...
  const cardHtml = items.map((t, i) => {
    ...
    return `<div class="card ${id}-card" style="width:19cqw;padding:2cqw;opacity:0;">
      ...
      <div class="display" style="font-size:2.3cqw;margin-top:1cqw;color:${theme.plum};">${esc(ct)}</div>
      ${cs ? `<div class="body" style="font-size:1.35cqw;margin-top:0.5cqw;">${esc(cs)}</div>` : ""}</div>`;
  }).join("");
...
function blRibbons(scene, ctx) {
  ...
  const html = `${open(id, ctx)}
    ...
    <div class="safe">
      ...
      <h1 class="display" id="${id}-em" style="font-style:italic;font-size:6cqw;margin-top:2.2cqw;opacity:0;">${hl(scene, theme.coral)}</h1>
      <svg id="${id}-fl" width="560" height="34" viewBox="0 0 560 34" style="overflow:visible;max-width:30cqw;">
