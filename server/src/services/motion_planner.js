...
function verifyMotion({ plan, indexHtml, storyboard } = {}) {
  ...
  //
  // BUT ONLY WHERE PER-SCENE CLIPS ARE THE ARCHITECTURE. This counts tweens aimed at `s1`…`sN`,
  // which assumes the composition animates each scene through its own clip. Two families do not:
  // lf_stage's 27 long-form packs and the lf-film reference films drive EVERYTHING from a single
  // proxy tween whose onUpdate redraws the whole frame from the playhead — by design, and the same
  // design that makes them seek-exact. Against that, every scene reports zero tweens and the film
  // is declared 45-of-45 static while it is in fact fully animated. Verified on 05-signal: 45/45
  // flagged, and the same false positive is visible on shipped ship-log and tide-table jobs.
  //
  // The header above already grants that "a dedicated composer owns its own choreography" for
  // `honored`; this extends the same honesty to `staticScenes`. If the composition emits NONE of
  // the clips this check looks for, the count is measuring something that was never there — so it
  // reports nothing rather than everything. A scene-kit film missing ONE clip still has the rest,
  // so it keeps the full check.
  // "Does this composition use per-scene clips AT ALL" — asked of the markup OR the selectors,
  // because a composition can carry either without the other: the scene-kit emits `<div id="s1">`
  // and tweens `"#s1"`, while a harness fixture may carry only the script half. Matching just the
  // markup made this skip a script-only fixture and stop naming its one genuinely static scene.
  const hasSceneClips = clipIds.some((id) => html.includes(`id="${id}"`) || html.includes(`#${id}`));
  const staticScenes = hasSceneClips ? scenes.filter((s) => s.tweens === 0).map((s) => s.sceneId) : [];
