---
name: allotment
label: Allotment
orientation: landscape
form: longform
fontFamily: Zilla Slab
---

# Allotment

Vegetable plot. A five-minute, 1920x1080 text-first film — forty kinetic-typography beats,
one continuous hand-drawn world behind all of them, and exactly two image slots in the entire run.

## Palette

| Role | Hex | Job |
|---|---|---|
| paper | `#f2f0e0` | the default ground |
| ink | `#22301f` | all type |
| accent | `#c4622c` | kickers, rules, emphasis — brandable |
| accent2 | `#4d8c3f` | the second voice — brandable |
| sageT | `#e6eedc` | tinted ground A, alternating |
| terraT | `#f6e3cd` | tinted ground B, alternating |
| desk | `#1e2a1a` | outside the frame |

## Type

Display **Zilla Slab**, body **Source Sans 3**. Headlines are authored at up to 210px and shrink only
when a line genuinely cannot fit; nothing is set below 24px.

## Motion

Per-scene camera from `["zoomOut","pushU","hopU","pushL","drop","pushR"]` at stride 3/2, entering over the
first 12% of a beat and leaving over the last 12%. A continuous drift keeps translate and scale
moving the whole time. Ambient clock multiplier 1.5.

## World

Rows of seedlings growing, watering can, hopping robin — computed from the clock,
running under every beat, hidden (never stopped) on the 15 scenes that ask for a bare field.

## Media

One 980x540 screenshot card at the Peek beat, one 140px circular logo at Join. That is the whole
appetite: `media.maxAssets` is 3. Everything else on screen is type and inline SVG.
