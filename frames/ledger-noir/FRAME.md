...
description: >
  ...
  One accent (#8FB4FF) marks a single focal device per frame; #C9CFDA is the secondary.
...
Ground `#0A0C10`. Text is high-contrast ink. `#8FB4FF` is the one saturated device per frame
(a rule, dot, underline, or the single CTA fill); `#C9CFDA` supports it. No third saturated hue in a
