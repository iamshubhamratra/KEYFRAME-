...
**Renderer** `showcase-vertical` · **Composer** `server/src/services/showcase_vertical_composer.js` · **Stage** 1080×1920 (`orientation: portrait`) · built on `om_port_kit`

The portrait sibling of Showcase: an annotated product tour re-authored for the tall frame rather than scaled into it. Near-white paper washed with soft colour blooms and a fine 52px grid. The phone becomes the natural device — it fills the height the way a browser window fills a wide frame — with copy stacked above it and numbered callouts landing ON the screen instead of beside it. The gallery beat stacks browser windows that snap in from alternating sides, and the closing figures ride full-width panel cards. Authored portrait-first (9:16) as a native GSAP + DOM composition. Best for mobile apps, SaaS and any product whose story IS its interface, cut for Reels, Shorts and Stories.

## Spine

`intro · tour · gallery · stats · cta`

Picture slots: **up to 5 (intro, tour, gallery×3)**. The first beat and the last are fixed; the middle beats walk their
roles in order, skipping any layout the beat cannot carry. A film may show three of the middle
layouts — never a reshuffle.

## Signature

Re-authored for the tall frame rather than scaled: the phone becomes the natural device, copy stacks above it, and callouts land ON the screen instead of beside it.
...
```
node scripts/om-port-harness.js showcase-vertical <outDir> "" shots:8
