# Hay & Holler — long-form kinetic-typography film (16:9)

A 5:11, 47-beat looping film for small farm / livestock, rendered from its own
original bundle — only the words and the two pictures change. Built for LONG-FORM
video: every beat is a different type animation (~6.6s each), so a 2–5 minute
script never repeats a layout.

## Design system
- Paper `#f7eed6`, ink `#2c2415`, accent `#d4832a`, second accent
  `#5e7a34`, tinted grounds `#e9eeda` / `#f9e2bd`.
- **Rye** display over **Roboto Condensed** body (bundled in the standalone).
...
- Animated SVG world behind every scene: tractor crossing the field, hay bales rolling, spinning sun wheel, crowing rooster.

## Shape vocabulary (47 authored beats)
