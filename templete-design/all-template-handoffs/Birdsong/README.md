# Birdsong — 9:16 animated template

Bold flat colour blocks with birds and trees — flapping flocks, perched birds, swaying trees, falling leaves, story blocks.

**1080x1920 · 7 scenes · ~13.4s**
...
```
standalone/Birdsong.html      ← ONE self-contained file. Open in any browser. No build, no server.
src/
  Birdsong.dc.html          ← entry page: fonts, resets, scene data (OM_SCENES), mount
  birdsong-film.jsx   ← THE TEMPLATE: all scenes + motion code
...
**Just want the video?** Use `standalone/Birdsong.html`.
...
# → http://localhost:8000/Birdsong.dc.html
```
...
Copy, timing and scene order live in one line of `Birdsong.dc.html`:
...
| Key | What it does |
| --- | --- |
| `name` | Which scene component renders it — must match a key in `MAP` (bottom of `birdsong-film.jsx`) |
...
**Change speed** — scale every `dur`. Currently ~13.4s; ×0.8 for faster, ×1.25 for slower.
**Reorder / duplicate / delete scenes** — move or remove array entries.
**Rebrand** — edit `window.OM_TWEAKS`. Tweakable: block 1, block 2, block 3, brand name, URL.

### Image slots

feature desktop card, 3 gallery desktop cards, logo.
...
`birdsong-film.jsx` is one file: motion helpers at the top (easing, `seg()`, the `M` preset
...
Write a component in `birdsong-film.jsx`, add it to `MAP`, then add an entry to `OM_SCENES`
with a matching `name`.

### Changing aspect ratio

In `birdsong-film.jsx`, find:
```jsx
...
Open `standalone/Birdsong.html` and screen-record at 1080×1920, or drive the
...
React 18 + Babel load from CDN (see `<head>` of `Birdsong.dc.html`) — swap for local
copies if you need offline builds. Fonts load from Google Fonts.
`standalone/Birdsong.html` has everything inlined already.
