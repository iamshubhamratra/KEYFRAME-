# Reel — 9:16 animated template

Social story — animated gradient, stories progress bar, kinetic word-highlight captions, sticker tags.

**1080x1920 · 6 scenes · ~14.6s**
...
```
standalone/Reel.html      ← ONE self-contained file. Open in any browser. No build, no server.
src/
  Reel.dc.html              ← entry page: fonts, resets, scene data (OM_SCENES), mount
  reel-film.jsx       ← THE TEMPLATE: all scenes + motion code
...
**Just want the video?** Use `standalone/Reel.html`.
...
# → http://localhost:8000/Reel.dc.html
```
...
Copy, timing and scene order live in one line of `Reel.dc.html`:
...
| Key | What it does |
| --- | --- |
| `name` | Which scene component renders it — must match a key in `MAP` (bottom of `reel-film.jsx`) |
...
**Change speed** — scale every `dur`. Currently ~14.6s; ×0.8 for faster, ×1.25 for slower.
**Reorder / duplicate / delete scenes** — move or remove array entries.
**Rebrand** — edit `window.OM_TWEAKS`. Tweakable: accent, gradient A, gradient B, brand name, handle.
...
`reel-film.jsx` is one file: motion helpers at the top (easing, `seg()`, the `M` preset
...
Write a component in `reel-film.jsx`, add it to `MAP`, then add an entry to `OM_SCENES`
with a matching `name`.

### Changing aspect ratio

In `reel-film.jsx`, find:
```jsx
...
Open `standalone/Reel.html` and screen-record at 1080×1920, or drive the
...
React 18 + Babel load from CDN (see `<head>` of `Reel.dc.html`) — swap for local
copies if you need offline builds. Fonts load from Google Fonts.
`standalone/Reel.html` has everything inlined already.
