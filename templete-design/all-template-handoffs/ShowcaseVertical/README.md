# ShowcaseVertical — 9:16 animated template

Vertical annotated product tour — arrows, numbered callouts, stacked screenshot montage.

**1080x1920 · 6 scenes · ~15.4s**
...
```
standalone/ShowcaseVertical.html      ← ONE self-contained file. Open in any browser. No build, no server.
src/
  ShowcaseVertical.dc.html  ← entry page: fonts, resets, scene data (OM_SCENES), mount
  showvert-film.jsx   ← THE TEMPLATE: all scenes + motion code
...
**Just want the video?** Use `standalone/ShowcaseVertical.html`.
...
# → http://localhost:8000/ShowcaseVertical.dc.html
```
...
Copy, timing and scene order live in one line of `ShowcaseVertical.dc.html`:
...
| Key | What it does |
| --- | --- |
| `name` | Which scene component renders it — must match a key in `MAP` (bottom of `showvert-film.jsx`) |
...
**Change speed** — scale every `dur`. Currently ~15.4s; ×0.8 for faster, ×1.25 for slower.
**Reorder / duplicate / delete scenes** — move or remove array entries.
**Rebrand** — edit `window.OM_TWEAKS`. Tweakable: accent, brand name, URL.

### Image slots

hero phone, tour desktop, detail desktop, 3 gallery cards, logo.
...
`showvert-film.jsx` is one file: motion helpers at the top (easing, `seg()`, the `M` preset
...
Write a component in `showvert-film.jsx`, add it to `MAP`, then add an entry to `OM_SCENES`
with a matching `name`.

### Changing aspect ratio

In `showvert-film.jsx`, find:
```jsx
...
Open `standalone/ShowcaseVertical.html` and screen-record at 1080×1920, or drive the
...
React 18 + Babel load from CDN (see `<head>` of `ShowcaseVertical.dc.html`) — swap for local
copies if you need offline builds. Fonts load from Google Fonts.
`standalone/ShowcaseVertical.html` has everything inlined already.
