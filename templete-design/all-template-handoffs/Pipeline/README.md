# Pipeline — 16:9 animated template

Assembly line — screenshots ride a working conveyor past turning gears, a robotic arm and a stamping press.

**1920x1080 · 6 scenes · ~16.7s**
...
```
standalone/Pipeline.html      ← ONE self-contained file. Open in any browser. No build, no server.
src/
  Pipeline.dc.html          ← entry page: fonts, resets, scene data (OM_SCENES), mount
  pipeline-film.jsx   ← THE TEMPLATE: all scenes + motion code
...
**Just want the video?** Use `standalone/Pipeline.html`.
...
# → http://localhost:8000/Pipeline.dc.html
```
...
Copy, timing and scene order live in one line of `Pipeline.dc.html`:
...
| Key | What it does |
| --- | --- |
| `name` | Which scene component renders it — must match a key in `MAP` (bottom of `pipeline-film.jsx`) |
...
**Change speed** — scale every `dur`. Currently ~16.7s; ×0.8 for faster, ×1.25 for slower.
...
3 belt screenshots, desktop screenshot, 4 assembly parts, logo.
...
`pipeline-film.jsx` is one file: motion helpers at the top (easing, `seg()`, the `M` preset
...
Write a component in `pipeline-film.jsx`, add it to `MAP`, then add an entry to `OM_SCENES`
with a matching `name`.

### Changing aspect ratio

In `pipeline-film.jsx`, find:
```jsx
...
Open `standalone/Pipeline.html` and screen-record at 1920×1080, or drive the
...
React 18 + Babel load from CDN (see `<head>` of `Pipeline.dc.html`) — swap for local
copies if you need offline builds. Fonts load from Google Fonts.
`standalone/Pipeline.html` has everything inlined already.
