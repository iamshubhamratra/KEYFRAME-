# Fight — 16:9 animated template

Fight-night pitch — arena spotlight, ring ropes, VS layout, KO impact bursts, judges scorecard, screen-shake cuts.

**1920x1080 · 6 scenes · ~19.3s**
...
```
standalone/Fight.html      ← ONE self-contained file. Open in any browser. No build, no server.
src/
  Fight.dc.html             ← entry page: fonts, resets, scene data (OM_SCENES), mount
  fight-film.jsx      ← THE TEMPLATE: all scenes + motion code
...
**Just want the video?** Use `standalone/Fight.html`.
...
# → http://localhost:8000/Fight.dc.html
```
...
Copy, timing and scene order live in one line of `Fight.dc.html`:
...
| Key | What it does |
| --- | --- |
| `name` | Which scene component renders it — must match a key in `MAP` (bottom of `fight-film.jsx`) |
...
**Change speed** — scale every `dur`. Currently ~19.3s; ×0.8 for faster, ×1.25 for slower.
**Reorder / duplicate / delete scenes** — move or remove array entries.
**Rebrand** — edit `window.OM_TWEAKS`. Tweakable: corner accent, belt gold, brand name, URL.

### Image slots

jumbotron screenshot, phone screen, logo.
...
`fight-film.jsx` is one file: motion helpers at the top (easing, `seg()`, the `M` preset
...
Write a component in `fight-film.jsx`, add it to `MAP`, then add an entry to `OM_SCENES`
with a matching `name`.

### Changing aspect ratio

In `fight-film.jsx`, find:
```jsx
...
Open `standalone/Fight.html` and screen-record at 1920×1080, or drive the
...
React 18 + Babel load from CDN (see `<head>` of `Fight.dc.html`) — swap for local
copies if you need offline builds. Fonts load from Google Fonts.
`standalone/Fight.html` has everything inlined already.
