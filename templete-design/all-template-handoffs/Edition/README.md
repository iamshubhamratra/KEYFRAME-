# Edition — 16:9 animated template

Swiss editorial print magazine — modular grid, Anton display, figure-captioned plates, rolling ledger, halftone press layer.

**1920x1080 · 6 scenes · ~18.8s**
...
```
standalone/Edition.html      ← ONE self-contained file. Open in any browser. No build, no server.
src/
  Edition.dc.html           ← entry page: fonts, resets, scene data (OM_SCENES), mount
  edition-film.jsx    ← THE TEMPLATE: all scenes + motion code
...
**Just want the video?** Use `standalone/Edition.html`.
...
# → http://localhost:8000/Edition.dc.html
```
...
Copy, timing and scene order live in one line of `Edition.dc.html`:
...
| Key | What it does |
| --- | --- |
| `name` | Which scene component renders it — must match a key in `MAP` (bottom of `edition-film.jsx`) |
...
**Change speed** — scale every `dur`. Currently ~18.8s; ×0.8 for faster, ×1.25 for slower.
**Reorder / duplicate / delete scenes** — move or remove array entries.
**Rebrand** — edit `window.OM_TWEAKS`. Tweakable: ink accent, masthead, URL.

### Image slots

cover plate, dashboard plate, 3 spread plates, logo plate.
...
`edition-film.jsx` is one file: motion helpers at the top (easing, `seg()`, the `M` preset
...
Write a component in `edition-film.jsx`, add it to `MAP`, then add an entry to `OM_SCENES`
with a matching `name`.

### Changing aspect ratio

In `edition-film.jsx`, find:
```jsx
...
Open `standalone/Edition.html` and screen-record at 1920×1080, or drive the
...
React 18 + Babel load from CDN (see `<head>` of `Edition.dc.html`) — swap for local
copies if you need offline builds. Fonts load from Google Fonts.
`standalone/Edition.html` has everything inlined already.
