# Drive — 16:9 animated template

Golden-hour highway drive — product screenshots are the hero, cars run an ambient strip along the bottom.

**1920x1080 · 6 scenes · ~15.9s**
...
```
standalone/Drive.html      ← ONE self-contained file. Open in any browser. No build, no server.
src/
  Drive.dc.html             ← entry page: fonts, resets, scene data (OM_SCENES), mount
  drive-film.jsx      ← THE TEMPLATE: all scenes + motion code
...
**Just want the video?** Use `standalone/Drive.html`.
...
# → http://localhost:8000/Drive.dc.html
```
...
Copy, timing and scene order live in one line of `Drive.dc.html`:
...
| Key | What it does |
| --- | --- |
| `name` | Which scene component renders it — must match a key in `MAP` (bottom of `drive-film.jsx`) |
...
**Change speed** — scale every `dur`. Currently ~15.9s; ×0.8 for faster, ×1.25 for slower.
**Reorder / duplicate / delete scenes** — move or remove array entries.
**Rebrand** — edit `window.OM_TWEAKS`. Tweakable: accent, car colour, brand name, URL.

### Image slots

hero desktop, screenshot, phone screen, 6 montage tiles, logo.
...
`drive-film.jsx` is one file: motion helpers at the top (easing, `seg()`, the `M` preset
...
Write a component in `drive-film.jsx`, add it to `MAP`, then add an entry to `OM_SCENES`
with a matching `name`.

### Changing aspect ratio

In `drive-film.jsx`, find:
```jsx
...
Open `standalone/Drive.html` and screen-record at 1920×1080, or drive the
...
React 18 + Babel load from CDN (see `<head>` of `Drive.dc.html`) — swap for local
copies if you need offline builds. Fonts load from Google Fonts.
`standalone/Drive.html` has everything inlined already.
