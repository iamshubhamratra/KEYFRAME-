# Showcase — 16:9 animated template

Annotated product tour — drawn arrows, numbered callouts, zoom-ins, cursor, 6-tile montage, paper planes and birds.

**1920x1080 · 7 scenes · ~17.5s**
...
```
standalone/Showcase.html      ← ONE self-contained file. Open in any browser. No build, no server.
src/
  Showcase.dc.html          ← entry page: fonts, resets, scene data (OM_SCENES), mount
  showcase-film.jsx   ← THE TEMPLATE: all scenes + motion code
...
**Just want the video?** Use `standalone/Showcase.html`.
...
# → http://localhost:8000/Showcase.dc.html
```
...
Copy, timing and scene order live in one line of `Showcase.dc.html`:
...
| Key | What it does |
| --- | --- |
| `name` | Which scene component renders it — must match a key in `MAP` (bottom of `showcase-film.jsx`) |
...
**Change speed** — scale every `dur`. Currently ~17.5s; ×0.8 for faster, ×1.25 for slower.
...
hero, dashboard, detail, phone, 6 montage tiles, logo.
...
`showcase-film.jsx` is one file: motion helpers at the top (easing, `seg()`, the `M` preset
...
Write a component in `showcase-film.jsx`, add it to `MAP`, then add an entry to `OM_SCENES`
with a matching `name`.

### Changing aspect ratio

In `showcase-film.jsx`, find:
```jsx
...
Open `standalone/Showcase.html` and screen-record at 1920×1080, or drive the
...
React 18 + Babel load from CDN (see `<head>` of `Showcase.dc.html`) — swap for local
copies if you need offline builds. Fonts load from Google Fonts.
`standalone/Showcase.html` has everything inlined already.
