# FetchVertical — 9:16 animated template

Vertical dog-fetch story — portrait park, run cycle, master catch, signboard screenshots.

**1080x1920 · 6 scenes · ~20.0s**
...
```
standalone/FetchVertical.html      ← ONE self-contained file. Open in any browser. No build, no server.
src/
  FetchVertical.dc.html     ← entry page: fonts, resets, scene data (OM_SCENES), mount
  fetch-film.jsx      ← shared vector/character kit this template reuses
  fetchvert-film.jsx  ← THE TEMPLATE: all scenes + motion code
...
**Just want the video?** Use `standalone/FetchVertical.html`.
...
# → http://localhost:8000/FetchVertical.dc.html
```
...
Copy, timing and scene order live in one line of `FetchVertical.dc.html`:
...
| Key | What it does |
| --- | --- |
| `name` | Which scene component renders it — must match a key in `MAP` (bottom of `fetchvert-film.jsx`) |
...
**Change speed** — scale every `dur`. Currently ~20.0s; ×0.8 for faster, ×1.25 for slower.
**Reorder / duplicate / delete scenes** — move or remove array entries.
**Rebrand** — edit `window.OM_TWEAKS`. Tweakable: accent, brand name, URL, dog coat.

### Image slots

title signboard, run signboard, phone screen, logo.
...
`fetchvert-film.jsx` is one file: motion helpers at the top (easing, `seg()`, the `M` preset
...
Write a component in `fetchvert-film.jsx`, add it to `MAP`, then add an entry to `OM_SCENES`
with a matching `name`.

### Changing aspect ratio

In `fetchvert-film.jsx`, find:
```jsx
...
Open `standalone/FetchVertical.html` and screen-record at 1080×1920, or drive the
...
React 18 + Babel load from CDN (see `<head>` of `FetchVertical.dc.html`) — swap for local
copies if you need offline builds. Fonts load from Google Fonts.
`standalone/FetchVertical.html` has everything inlined already.
