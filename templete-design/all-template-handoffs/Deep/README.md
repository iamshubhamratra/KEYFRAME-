# Deep — 16:9 animated template

Bioluminescent deep sea — glowing viewport panels, caustic light, jellyfish, bubbles, sonar rings.
...
```
standalone/Deep.html      ← ONE self-contained file. Open in any browser. No build, no server.
src/
  Deep.dc.html              ← entry page: fonts, resets, scene data (OM_SCENES), mount
  deep-film.jsx       ← THE TEMPLATE: all scenes + motion code
...
**Just want the video?** Use `standalone/Deep.html`.
...
# → http://localhost:8000/Deep.dc.html
```
...
Copy, timing and scene order live in one line of `Deep.dc.html`:
...
| Key | What it does |
| --- | --- |
| `name` | Which scene component renders it — must match a key in `MAP` (bottom of `deep-film.jsx`) |
...
**Rebrand** — edit `window.OM_TWEAKS`. Tweakable: bioluminescence, creature glow, brand name, URL.

### Image slots

hero desktop, 5 viewport panels, phone screen, logo.
...
`deep-film.jsx` is one file: motion helpers at the top (easing, `seg()`, the `M` preset
...
Write a component in `deep-film.jsx`, add it to `MAP`, then add an entry to `OM_SCENES`
with a matching `name`.

### Changing aspect ratio

In `deep-film.jsx`, find:
```jsx
...
Open `standalone/Deep.html` and screen-record at 1920×1080, or drive the
...
React 18 + Babel load from CDN (see `<head>` of `Deep.dc.html`) — swap for local
copies if you need offline builds. Fonts load from Google Fonts.
`standalone/Deep.html` has everything inlined already.
