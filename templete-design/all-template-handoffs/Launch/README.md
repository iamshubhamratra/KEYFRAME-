# Launch — 16:9 animated template

Product launch in the Fetch park style — the dog acts as mascot presenting the reveal.

**1920x1080 · 6 scenes · ~16.9s**
...
```
standalone/Launch.html      ← ONE self-contained file. Open in any browser. No build, no server.
src/
  Launch.dc.html            ← entry page: fonts, resets, scene data (OM_SCENES), mount
  fetch-film.jsx      ← shared vector/character kit this template reuses
  launch-film.jsx     ← THE TEMPLATE: all scenes + motion code
...
**Just want the video?** Use `standalone/Launch.html`.
...
# → http://localhost:8000/Launch.dc.html
```
...
Copy, timing and scene order live in one line of `Launch.dc.html`:
...
| Key | What it does |
| --- | --- |
| `name` | Which scene component renders it — must match a key in `MAP` (bottom of `launch-film.jsx`) |
...
**Change speed** — scale every `dur`. Currently ~16.9s; ×0.8 for faster, ×1.25 for slower.
**Reorder / duplicate / delete scenes** — move or remove array entries.
**Rebrand** — edit `window.OM_TWEAKS`. Tweakable: accent, brand name, URL, mascot coat.

### Image slots

product screenshot, desktop screenshot, phone screen, logo.
...
`launch-film.jsx` is one file: motion helpers at the top (easing, `seg()`, the `M` preset
...
Write a component in `launch-film.jsx`, add it to `MAP`, then add an entry to `OM_SCENES`
with a matching `name`.

### Changing aspect ratio

In `launch-film.jsx`, find:
```jsx
...
Open `standalone/Launch.html` and screen-record at 1920×1080, or drive the
...
React 18 + Babel load from CDN (see `<head>` of `Launch.dc.html`) — swap for local
copies if you need offline builds. Fonts load from Google Fonts.
`standalone/Launch.html` has everything inlined already.
