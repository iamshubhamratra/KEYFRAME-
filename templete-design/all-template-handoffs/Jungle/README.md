# Jungle — 16:9 animated template

Layered rainforest — parallax canopy, swinging monkey, flapping toucan, prowling tiger, fireflies. Screenshots on wooden signboards.
...
```
standalone/Jungle.html      ← ONE self-contained file. Open in any browser. No build, no server.
src/
  Jungle.dc.html            ← entry page: fonts, resets, scene data (OM_SCENES), mount
  jungle-film.jsx     ← THE TEMPLATE: all scenes + motion code
...
**Just want the video?** Use `standalone/Jungle.html`.
...
# → http://localhost:8000/Jungle.dc.html
```
...
Copy, timing and scene order live in one line of `Jungle.dc.html`:
...
| Key | What it does |
| --- | --- |
| `name` | Which scene component renders it — must match a key in `MAP` (bottom of `jungle-film.jsx`) |
...
signboard screenshot, phone screen, 5 sighting boards, logo.
...
`jungle-film.jsx` is one file: motion helpers at the top (easing, `seg()`, the `M` preset
...
Write a component in `jungle-film.jsx`, add it to `MAP`, then add an entry to `OM_SCENES`
with a matching `name`.

### Changing aspect ratio

In `jungle-film.jsx`, find:
```jsx
...
Open `standalone/Jungle.html` and screen-record at 1920×1080, or drive the
...
React 18 + Babel load from CDN (see `<head>` of `Jungle.dc.html`) — swap for local
copies if you need offline builds. Fonts load from Google Fonts.
`standalone/Jungle.html` has everything inlined already.
