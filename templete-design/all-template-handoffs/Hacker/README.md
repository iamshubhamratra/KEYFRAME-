# Hacker — 16:9 animated template

Terminal/hacker vibe — matrix rain, scanlines, glitch titles, self-typing syntax-highlighted code.

**1920x1080 · 6 scenes · ~16.6s**
...
```
standalone/Hacker.html      ← ONE self-contained file. Open in any browser. No build, no server.
src/
  Hacker.dc.html            ← entry page: fonts, resets, scene data (OM_SCENES), mount
  hacker-film.jsx     ← THE TEMPLATE: all scenes + motion code
...
**Just want the video?** Use `standalone/Hacker.html`.
...
# → http://localhost:8000/Hacker.dc.html
```
...
Copy, timing and scene order live in one line of `Hacker.dc.html`:
...
| Key | What it does |
| --- | --- |
| `name` | Which scene component renders it — must match a key in `MAP` (bottom of `hacker-film.jsx`) |
...
**Change speed** — scale every `dur`. Currently ~16.6s; ×0.8 for faster, ×1.25 for slower.
**Reorder / duplicate / delete scenes** — move or remove array entries.
**Rebrand** — edit `window.OM_TWEAKS`. Tweakable: phosphor colour, brand name, prompt host.

### Image slots

preview screenshot, 2 node screenshots, logo.
...
`hacker-film.jsx` is one file: motion helpers at the top (easing, `seg()`, the `M` preset
...
Write a component in `hacker-film.jsx`, add it to `MAP`, then add an entry to `OM_SCENES`
with a matching `name`.

### Changing aspect ratio

In `hacker-film.jsx`, find:
```jsx
...
Open `standalone/Hacker.html` and screen-record at 1920×1080, or drive the
...
React 18 + Babel load from CDN (see `<head>` of `Hacker.dc.html`) — swap for local
copies if you need offline builds. Fonts load from Google Fonts.
`standalone/Hacker.html` has everything inlined already.
