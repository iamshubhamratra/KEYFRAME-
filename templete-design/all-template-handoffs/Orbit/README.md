# Orbit — 16:9 animated template

Rocket launch to orbit — starfield, ignition, stage separation, orbital arc, mission telemetry.

**1920x1080 · 6 scenes · ~17.0s**
...
```
standalone/Orbit.html      ← ONE self-contained file. Open in any browser. No build, no server.
src/
  Orbit.dc.html             ← entry page: fonts, resets, scene data (OM_SCENES), mount
  orbit-film.jsx      ← THE TEMPLATE: all scenes + motion code
...
**Just want the video?** Use `standalone/Orbit.html`.
...
# → http://localhost:8000/Orbit.dc.html
```
...
Copy, timing and scene order live in one line of `Orbit.dc.html`:
...
| Key | What it does |
| --- | --- |
| `name` | Which scene component renders it — must match a key in `MAP` (bottom of `orbit-film.jsx`) |
...
**Change speed** — scale every `dur`. Currently ~17.0s; ×0.8 for faster, ×1.25 for slower.
...
desktop screenshots, phone screen, logo.
...
`orbit-film.jsx` is one file: motion helpers at the top (easing, `seg()`, the `M` preset
...
Write a component in `orbit-film.jsx`, add it to `MAP`, then add an entry to `OM_SCENES`
with a matching `name`.

### Changing aspect ratio

In `orbit-film.jsx`, find:
```jsx
...
Open `standalone/Orbit.html` and screen-record at 1920×1080, or drive the
...
React 18 + Babel load from CDN (see `<head>` of `Orbit.dc.html`) — swap for local
copies if you need offline builds. Fonts load from Google Fonts.
`standalone/Orbit.html` has everything inlined already.
