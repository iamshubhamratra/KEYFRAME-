# Stomp — 9:16 animated template

Energetic stomp typography office ad — giant word slams, letter cascades, scrolling tickers, animated office people (typing, walking, calling, waving).

**1080x1920 · 14 scenes · ~35.8s**
...
```
standalone/Stomp.html      ← ONE self-contained file. Open in any browser. No build, no server.
src/
  Stomp.dc.html             ← entry page: fonts, resets, scene data (OM_SCENES), mount
  stomp-film.jsx      ← THE TEMPLATE: all scenes + motion code
...
**Just want the video?** Use `standalone/Stomp.html`.
...
# → http://localhost:8000/Stomp.dc.html
```
...
Copy, timing and scene order live in one line of `Stomp.dc.html`:
...
| Key | What it does |
| --- | --- |
| `name` | Which scene component renders it — must match a key in `MAP` (bottom of `stomp-film.jsx`) |
...
**Change speed** — scale every `dur`. Currently ~35.8s; ×0.8 for faster, ×1.25 for slower.
...
2 desktop screenshots, office photo, logo.
...
`stomp-film.jsx` is one file: motion helpers at the top (easing, `seg()`, the `M` preset
...
Write a component in `stomp-film.jsx`, add it to `MAP`, then add an entry to `OM_SCENES`
with a matching `name`.

### Changing aspect ratio

In `stomp-film.jsx`, find:
```jsx
...
Open `standalone/Stomp.html` and screen-record at 1080×1920, or drive the
...
React 18 + Babel load from CDN (see `<head>` of `Stomp.dc.html`) — swap for local
copies if you need offline builds. Fonts load from Google Fonts.
`standalone/Stomp.html` has everything inlined already.
