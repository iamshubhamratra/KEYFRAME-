...
window.FilmKit = (function () {
  ...
  function make(cfg) {
    ...
    /* ── PATCH: type scale ─────────────────────────────────────────────
       One tweak (Compact | Default | Editorial) moves size, measure and
       leading together. SCALE is set by Root at the top of every render pass,
       before any scene renders, so it never depends on the playhead. */
    ...
    /* ── PATCH: the empty-slot swap map ───────────────────────────────
       A media beat keeps its slot. When the slot comes back empty the beat
       renders as the pack's look.swap beat ("quote" | "body") instead of a
       dashed placeholder, borrowing the nearest unclaimed line. The map is a
       pure function of the whole authored scene list — computed once, never
       from the playhead — so a seek cannot change which line a beat shows.
    ...
    let LIST = null, SWAP = null;
    const sceneList = () => {
      if (!LIST) { try { LIST = JSON.parse(window.OM_SCENES) || []; } catch (e) { LIST = []; } }
      return LIST;
    };
    function swapMap() {
      if (SWAP) return SWAP;
      const list = sceneList(), claimed = {}, out = {};
      list.forEach((s, i) => {
        const hasSlot = ("image" in s) || ("images" in s);
        if (!hasSlot) return;
        if (s.image || (s.images || []).some(Boolean)) return;
        let line = s.fallbackText || s.quote || s.body || s.sub || "";
        if (!line || claimed[line]) {
          line = "";
          for (let d = 1; d < list.length && !line; d++) {
            for (const n of [list[i + d], list[i - d]]) {
              const c = n && (n.fallbackText || n.sub);
              if (c && !claimed[c]) { line = c; break; }
            }
          }
        }
        if (line) { claimed[line] = 1; out[i] = line; }
      });
      SWAP = out;
      return SWAP;
    }
    const swapText = (idx, scene) => swapMap()[idx] || scene.fallbackText || scene.sub || "";
    ...
    function Chrome({ theme, fg }) {
      /* PATCH: cfg.chrome === false hides the template lockup entirely — a
         generated video should carry the user's brand, never the pack name. */
      if (cfg.chrome === false) return null;
    ...
    const Title = (txt, p, fg, hi, size, from, upper) =>
      splitLines(txt).map((ln, i) =>
        React.createElement("div", { key: i, style: { ...tM(p, (from || 0) + i), fontFamily: FH, fontSize: sz(size), lineHeight: (cfg.titleLine || 1.04) * SCALE.lh, color: i === 1 ? hi : fg, textTransform: upper ? "uppercase" : "none", letterSpacing: cfg.titleSpace || "0" } }, ln));
    ...
    function Feature({ progress: p, scene, theme }) {
      ...
      const sc0 = window.useScene ? window.useScene() : { index: 0 };
      /* PATCH: empty slot -> the pack's declared swap beat, never a placeholder. */
      if (L.swap && cfg.media !== false && !L.labelsOnly && !scene.image) {
        const S2 = L.swap === "quote" ? Quote : Body;
        const line = swapText(sc0.index, scene);
        return React.createElement(S2, { progress: p, theme, scene: { ...scene, quote: line, body: line, attrib: scene.attrib } });
      }
      return React.createElement(Frame, { progress: p, theme, bg, fg, world: L.world },
    ...
    function Montage({ progress: p, scene, theme }) {
      ...
      const sc0 = window.useScene ? window.useScene() : { index: 0 };
      /* PATCH: same swap contract as Feature — a comparison beat with no
         imagery becomes the pack's swap beat rather than four dashed boxes. */
      if (L.swap && cfg.media !== false && !L.labelsOnly && !((scene.images || []).some(Boolean))) {
        const S2 = L.swap === "quote" ? Quote : Body;
        const line = swapText(sc0.index, scene);
        return React.createElement(S2, { progress: p, theme, scene: { ...scene, quote: line, body: line } });
      }
          ...
          React.createElement("div", { style: { marginTop: 54, display: "grid", gridTemplateColumns: "1fr 1fr", gap: 26 } },
            tiles.slice(0, 4).map((label, i) =>
              React.createElement("div", { key: i, style: { ...iM(p, i + 2), transform: `rotate(${tilts[i]}deg)` } },
                (cfg.media === false || L.labelsOnly)
                  ? React.createElement("div", { style: { height: L.tile.h || 250, borderRadius: L.tile.r == null ? 22 : L.tile.r, background: tbg, display: "flex", alignItems: "center", justifyContent: "center", fontFamily: FH, fontSize: sz(L.tile.labelSize || 52), color: tlabel, textTransform: L.upper ? "uppercase" : "none", padding: 24, textAlign: "center", lineHeight: 1.08, border: L.tile.line ? `2px solid ${rgba(tline, 0.35)}` : "none" } }, label)
                  : React.createElement(React.Fragment, null,
    ...
    function CTA({ progress: p, scene, theme }) {
      ...
      return React.createElement(Frame, { progress: p, theme, bg, fg, world: L.world },
        React.createElement("div", { style: { position: "absolute", left: PAD, right: PAD, top: L.top || 470, textAlign: center ? "center" : "left" } },
          React.createElement("div", { style: { ...iM(p, 0), width: 148, height: 148, borderRadius: L.logoShape === "rounded" ? 20 : 999, overflow: "hidden", background: scene.logo ? rgba(fg, 0.08) : rgba(fg, 0.14), border: scene.logo ? `2px dashed ${rgba(fg, 0.4)}` : "none", margin: center ? "0 auto 42px" : "0 0 42px", display: "flex", alignItems: "center", justifyContent: "center" } },
            /* PATCH: no logo supplied -> the pack's own mark, never a dashed box. */
            scene.logo
              ? React.createElement(MediaSlot, { src: scene.logo, radius: L.logoShape === "rounded" ? 14 : 999, compact: true, line: fg })
              : React.createElement("div", { style: { transform: "scale(3.1)", display: "flex" } }, cfg.icon ? cfg.icon({ ...theme, currentBg: fg }, fg, React.createElement) : null)),
          Title(scene.title, p, fg, hi, L.size || 126, 1, L.upper),
    ...
    /* ── PATCH: Body — the article workhorse (20–60 words) ─────────────
       Measure is declared in characters (45–75) and honoured through the type
       scale; the entrance is a word cascade derived from progress, so a seek
       to any timestamp reproduces the same partially-revealed paragraph. */
    function Body({ progress: p, scene, theme }) {
      const L = cfg.look.body || cfg.look.statement, bg = col(theme, L.bg), fg = col(theme, L.fg), hi = col(theme, L.hi);
      const words = String(scene.body || scene.sub || "").split(/\s+/).filter(Boolean);
      const per = L.stagger == null ? 0.011 : L.stagger;
      const scB = window.useScene ? window.useScene() : { index: 0, total: 1 };
      const nTot = sceneList().length || scB.total || 1;
      const gutter = L.gutter === false ? null : (scene.mark || String(scB.index + 1).padStart(2, "0"));
      const rule = ease.outQuint(clamp01(PACE(p) / 0.22));
      return React.createElement(Frame, { progress: p, theme, bg, fg, world: L.world },
        React.createElement("div", { style: { position: "absolute", left: PAD, right: PAD, top: L.top || 300 } },
          scene.kicker && React.createElement(Kicker, { k: { ...(L.kicker || { v: "bare", c: L.hi }), text: scene.kicker }, p, theme }),
          scene.title ? Title(scene.title, p, fg, hi, L.size || 88, 0, L.upper) : null,
          React.createElement("div", { style: { marginTop: scene.title ? 42 : 18, height: 3, background: rgba(hi, 0.85), transform: `scaleX(${rule.toFixed(3)})`, transformOrigin: "left", width: 120 } }),
          React.createElement("div", { style: { display: "flex", gap: 26, marginTop: 34 } },
            gutter && React.createElement("div", { style: { ...PRESETS.rise(p, 1), fontFamily: cfg.FM || FB, fontWeight: 700, fontSize: sz(24), lineHeight: 1.6, color: rgba(fg, 0.58), minWidth: 44 } }, gutter),
            React.createElement("div", { style: { maxWidth: measure(L.ch) + "ch", fontFamily: FB, fontWeight: L.weight || 500, fontSize: sz(L.bodySize || 44), lineHeight: (L.lh || 1.5) * SCALE.lh, color: rgba(fg, L.dim || 0.9) } },
              words.map((w, i) => {
                const t = ease.outCubic(clamp01((PACE(p) - 0.05 - i * per) / 0.15));
                return React.createElement("span", { key: i, style: { display: "inline-block", opacity: t, transform: `translateY(${((1 - t) * 15).toFixed(2)}px)`, filter: t < 0.99 ? `blur(${((1 - t) * 5).toFixed(2)}px)` : "none" } }, w, "\u00a0");
              }))),
          scene.note && React.createElement("div", { style: { ...PRESETS.rise(p, 7), fontFamily: cfg.FM || FB, fontWeight: 700, fontSize: sz(26), letterSpacing: "0.06em", color: rgba(fg, 0.78), marginTop: 46, paddingLeft: gutter ? 70 : 0, display: "flex", alignItems: "center", gap: 14 } },
            React.createElement("span", { style: { width: 26, height: 4, borderRadius: 2, background: rgba(hi, 0.9) } }),
            scene.note)),
        L.footer === false ? null : React.createElement("div", { style: { position: "absolute", left: PAD, right: PAD, bottom: 150, display: "flex", alignItems: "center", gap: 24, opacity: rule } },
          React.createElement("div", { style: { flex: 1, height: 2, background: rgba(fg, 0.14) } },
            React.createElement("div", { style: { width: (((scB.index + 1) / Math.max(1, nTot)) * 100).toFixed(1) + "%", height: "100%", background: rgba(hi, 0.9) } })),
          React.createElement("div", { style: { fontFamily: cfg.FM || FB, fontWeight: 600, fontSize: sz(23), letterSpacing: "0.1em", color: rgba(fg, 0.42) } }, String(scB.index + 1).padStart(2, "0") + " / " + String(nTot).padStart(2, "0"))));
    }

    /* ── PATCH: Quote — the expressive beat, biggest typographic moment ── */
    function Quote({ progress: p, scene, theme }) {
      const L = cfg.look.quote || cfg.look.statement, bg = col(theme, L.bg), fg = col(theme, L.fg), hi = col(theme, L.hi);
      const lines = splitLines(scene.quote || scene.title || "");
      const attrib = scene.attrib || scene.sub;
      const inset = L.rule === false ? 0 : 34;
      return React.createElement(Frame, { progress: p, theme, bg, fg, world: L.world },
        React.createElement("div", { style: { position: "absolute", left: PAD, right: PAD, top: L.top || 360 } },
          L.mark !== false && React.createElement("div", { style: { ...PRESETS.rise(p, 0), fontFamily: FH, fontSize: sz(L.markSize || 210), lineHeight: 0.62, color: rgba(hi, 0.45) } }, "\u201C"),
          React.createElement("div", { style: { display: "flex", flexDirection: "column", gap: 8, marginTop: 14, borderLeft: inset ? `6px solid ${rgba(hi, 0.8)}` : "none", paddingLeft: inset } },
            lines.map((ln, i) => {
              const t = ease.outQuint(clamp01((PACE(p) - 0.07 - i * 0.085) / 0.33));
              return React.createElement("div", { key: i, style: { fontFamily: FH, fontSize: sz(L.size || 86), lineHeight: (L.lh || 1.16) * SCALE.lh, letterSpacing: cfg.titleSpace || "0", color: L.altLine && i % 2 ? hi : fg, maxWidth: measure(L.ch || 30) + "ch", clipPath: `inset(0 ${((1 - t) * 100).toFixed(2)}% 0 0)`, opacity: clamp01(t * 5) } }, ln);
            })),
          attrib && React.createElement("div", { style: { ...PRESETS.rise(p, 6), fontFamily: FB, fontWeight: 800, fontSize: sz(29), letterSpacing: "0.14em", textTransform: "uppercase", color: rgba(fg, 0.6), marginTop: 46, paddingLeft: inset ? inset + 6 : 0 } }, attrib)));
    }

    const SCENE_MAP = { Hook, Statement, Body, Quote, Feature, Montage, Stats, CTA, Cursor, Typing, Code, Scroll, DragDrop, Toggle, Notify, Swipe, Morph, Ring };
    ...
    function Root() {
      ...
      /* PATCH: resolve the type scale before any scene renders. */
      ...
      return React.createElement("div", { style: { width: "100%", height: "100%", display: "flex", alignItems: "center", justifyContent: "center", background: cfg.desk } },
        ...
        React.createElement(TP, null,
          ...
          React.createElement(TS, { label: "Type" }),
          React.createElement(TR, { label: "Scale", value: theme.typeScale, options: ["Compact", "Default", "Editorial"], onChange: (v) => setTweak("typeScale", v) })));
    }
