...
export default function CreateScreen({ onCreated }) {
  ...
  return (
    <div className="max-w-3xl mx-auto px-6 pt-16 pb-24">
      ...
      {/* Morphing input bar */}
      <div className="mt-10 rounded-2xl border border-line bg-panel overflow-hidden">
        <div className="flex border-b border-line">
          {TABS.map((t) => (
            ...
            >
              {tab === t.key && (
                <motion.div layoutId="tab-pill" className="absolute inset-0 bg-line" transition={{ type: "spring", stiffness: 500, damping: 40 }} />
              )}
          ...
          <AnimatePresence mode="wait">
            {tab === "prompt" && (
              <motion.textarea
                ...
                className="w-full resize-none bg-transparent outline-none text-lg placeholder:text-dim/50"
              />
            )}
            {tab === "url" && (
              <motion.div key="url" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }}>
                <input
                  ...
                  className="w-full bg-transparent outline-none text-lg placeholder:text-dim/50 py-2"
                />
                <textarea
                  value={prompt}
                  onChange={(e) => setPrompt(e.target.value)}
                  placeholder="Optional: anything specific you want the video to say?"
                  className="mt-3 w-full h-16 resize-none bg-transparent outline-none text-sm placeholder:text-dim/40 border-t border-line pt-3"
                />
              </motion.div>
            )}
            {tab === "video" && (
      ...
      <div className="mt-6 grid grid-cols-1 sm:grid-cols-3 gap-4">
          ...
          <div className="flex gap-2">
            {["horizontal", "vertical", "square"].map((o) => (
              <button key={o} onClick={() => setOrientation(o)}
                className={`flex items-center justify-center rounded border transition-all ${orientation === o ? "border-accent" : "border-line hover:border-dim"}`}
        ...
        <div className="rounded-xl border border-line bg-panel p-4">
          <label className="flex items-center gap-3 cursor-pointer">
            <input type="checkbox" checked={autopilot} onChange={(e) => setAutopilot(e.target.checked)} className="accent-[#ccff00]" />
            <div>
      ...
      <motion.button
        ...
        disabled={!canSubmit}
        onClick={submit}
        className={`mt-8 w-full py-4 rounded-xl font-display font-bold uppercase tracking-widest text-sm transition-colors ${canSubmit ? "bg-accent text-ground hover:bg-accent-dim" : "bg-line text-dim cursor-not-allowed"}`}
      >
...
function PackCard({ active, onClick, name, vibe, style }) {
  return (
    <button
      onClick={onClick}
      className={`rounded-xl border px-4 py-3 text-left transition-all min-w-40 ${active ? "border-accent" : "border-line hover:border-dim"}`}
      style={style ? { background: style.bg } : {}}
    >
      <div className="text-xs font-bold uppercase tracking-wider" style={style ? { color: style.label } : {}}>
        {name}
      </div>
      {vibe && <div className="text-[10px] mt-0.5" style={style ? { color: style.label, opacity: 0.65 } : { color: "var(--color-dim)" }}>{vibe}</div>}
      {style && (
        <div className="flex gap-1 mt-2">
          {style.chips.map((c) => <span key={c} className="w-3 h-3 rounded-full border border-black/30" style={{ background: c }} />)}
        </div>
      )}
    </button>
  );
