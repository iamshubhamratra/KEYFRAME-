---
version: alpha
name: Cats Playing Launch — Frame (video / frame layer)
description: >
  ...
  Grounds are dark (#0f0c1b); display type is JetBrains Mono, body is Inter.
  One accent (#ff9e2c) marks a single focal device per frame; #ff3385 is the secondary.
...
# Cats Playing Launch — Frame (video / frame layer)

## Overview

Cats Playing Launch is a retro-computer terminal: monospace type glowing on a dark CRT ground, scanline grid, one phosphor accent — for dev tools, CLIs, AI agents, hacker-grade launches. A dark retro-terminal announcement style featuring amber phosphor and neon magenta accents for playful launch claims. The ground is dark (#0f0c1b); text is
ink #eadbf7 on it. Exactly **one accent
device** (#ff9e2c) per frame marks the single focal point; #ff3385 is a supporting hue. Restraint carries
the identity — a clear type hierarchy, generous space, and one saturated moment beat a busy frame.

## Colors

Ground `#0f0c1b`. Text is high-contrast ink. `#ff9e2c` is the one saturated device per frame
(a rule, dot, underline, or the single CTA fill); `#ff3385` supports it. No third saturated hue in a
