---
version: alpha
name: Make A Templete Like C Launch — Frame (video / frame layer)
description: >
  ...
  Grounds are dark (#140f1d); display type is Space Grotesk, body is Inter.
  One accent (#ff9e3b) marks a single focal device per frame; #c084fc is the secondary.
...
# Make A Templete Like C Launch — Frame (video / frame layer)

## Overview

Make A Templete Like C Launch is a retro-computer terminal: monospace type glowing on a dark CRT ground, scanline grid, one phosphor accent — for dev tools, CLIs, AI agents, hacker-grade launches. A cozy nocturnal terminal aesthetic for cat-themed product reveals and home tech announcements. The ground is dark (#140f1d); text is
ink #f5ebf2 on it. Exactly **one accent
device** (#ff9e3b) per frame marks the single focal point; #c084fc is a supporting hue. Restraint carries
the identity — a clear type hierarchy, generous space, and one saturated moment beat a busy frame.

## Colors

Ground `#140f1d`. Text is high-contrast ink. `#ff9e3b` is the one saturated device per frame
(a rule, dot, underline, or the single CTA fill); `#c084fc` supports it. No third saturated hue in a
...
> Recipe: dark ground + grid texture · one accent device · clear Space Grotesk display moment · generous space.
...
- Set display in Space Grotesk, fit-to-measure; body in Inter; labels in JetBrains Mono caps.
