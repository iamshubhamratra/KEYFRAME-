---
version: alpha
name: Ledger Grid — Frame (video / frame layer)
description: >
  ...
  Grounds are dark (#0d1117); display type is IBM Plex Mono, body is Inter.
  One accent (#4c85f8) marks a single focal device per frame; #d8a93c is the secondary.
...
# Ledger Grid — Frame (video / frame layer)

## Overview

Ledger Grid is a chart-first analyst system: near-monochrome ground, a faint data grid, mono labels and ONE signal accent reserved for the data — for dashboards, analytics, data and ML products. A measured analytics pack — near-monochrome ground, a visible measurement grid and one ink-blue signal for figures that count up. The ground is dark (#0d1117); text is
ink #e6ecf5 on it. Exactly **one accent
device** (#4c85f8) per frame marks the single focal point; #d8a93c is a supporting hue. Restraint carries
the identity — a clear type hierarchy, generous space, and one saturated moment beat a busy frame.

## Colors

Ground `#0d1117`. Text is high-contrast ink. `#4c85f8` is the one saturated device per frame
(a rule, dot, underline, or the single CTA fill); `#d8a93c` supports it. No third saturated hue in a
