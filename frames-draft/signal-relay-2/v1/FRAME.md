---
version: alpha
name: Signal Relay 2 — Frame (video / frame layer)
description: >
  ...
  Grounds are dark (#080d0b); display type is IBM Plex Mono, body is Inter.
  One accent (#35e39a) marks a single focal device per frame; #5fa8ff is the secondary.
...
# Signal Relay 2 — Frame (video / frame layer)

## Overview

Signal Relay 2 is a retro-computer terminal: monospace type glowing on a dark CRT ground, scanline grid, one phosphor accent — for dev tools, CLIs, AI agents, hacker-grade launches. A near-black terminal pack with one phosphor accent and cursor-like motion, built for CLI tools, SDKs and API launches. The ground is dark (#080d0b); text is
ink #d2f5e2 on it. Exactly **one accent
device** (#35e39a) per frame marks the single focal point; #5fa8ff is a supporting hue. Restraint carries
the identity — a clear type hierarchy, generous space, and one saturated moment beat a busy frame.

## Colors

Ground `#080d0b`. Text is high-contrast ink. `#35e39a` is the one saturated device per frame
(a rule, dot, underline, or the single CTA fill); `#5fa8ff` supports it. No third saturated hue in a
